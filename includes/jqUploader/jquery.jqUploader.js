 ...
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		var opts = jQuery.extend({
				...
				src:    '/pegasus/includes/jqUploader/jqUploader.swf',
		...
		TerminateJQUploader = function(containerId,filename,varname){
			...
			$this.text('').append('<span style="color:#333333">Upload of <strong>'+filename+'</strong> finished! (the filename is now stored in the form as an hidden input field)</span><input name="'+varname+'" type="hidden" id="'+varname+'" value="'+filename+'"/>');
