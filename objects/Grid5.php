<?php
	...
	class Grid5 implements iGrid5 {
		...
		public function &createColumn($strTitle,$strColumn='') {
			$column = new Grid5Column($strTitle,$strColumn);
...
//			}
		...
		public function getInitialMarkup() {
			...
			$xhtml = "<table class=\"table table-striped table-bordered\" id=\"{$this->getId()}\">";
			$xhtml .= "<thead>";
			$xhtml .= "<tr>";
			$xhtml .= $xhtmlHeader;
			$xhtml .= "</tr>";
			...
			$xhtml .= "\t\t		\"fnStateSave\": function (oSettings, oData) { localStorage.setItem( 'DataTables-{$this->getId()}".Session::id()."', JSON.stringify(oData) ); },\n";
			$xhtml .= "\t\t		\"fnStateLoad\": function (oSettings) { return JSON.parse( localStorage.getItem('DataTables-{$this->getId()}".Session::id()."') ); },";
